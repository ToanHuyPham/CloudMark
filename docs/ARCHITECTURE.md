...
`network-peer-standard` uses `network-v5`: fixed pre/post route and structured
interface-counter snapshots, route/interface/MTU probes, read-only NIC
...
before it starts a child process. Network-v2 through network-v4 results remain
readable as legacy evidence but do not claim the v5 counter-window contract.
