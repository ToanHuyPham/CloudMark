...
DATABASE_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "mysql-peer-quick": {
        "label": "MySQL/MariaDB Peer Quick",
        "description": "Short authenticated Sysbench OLTP read-only and durable read/write baseline against an isolated InnoDB dataset.",
        "estimated_minutes": 6,
        "requires_agents": 2,
        "engine": "mysql",
        "port": 57306,
        ...
        "jobs": [
            {"name": "read-only-c1", "workload": "oltp_read_only", "threads": 1, "duration": 15, "warmup": 3},
            {"name": "read-write-c4", "workload": "oltp_read_write", "threads": 4, "duration": 30, "warmup": 5},
        ],
    },
    "mysql-peer-standard": {
        "label": "MySQL/MariaDB Peer Standard",
        "description": "Authenticated InnoDB point-select, read-only, write-only, and durable read/write concurrency with P99 latency and Generator CPU validity.",
        "estimated_minutes": 14,
        "requires_agents": 2,
        "engine": "mysql",
        "port": 57306,
        "tables": 8,
        "table_size": 50_000,
        "estimated_dataset_bytes": 512 * 1024 * 1024,
        "profile_version": "1.0",
        "methodology_version": "database-mysql-v1",
        "jobs": [
            {"name": "point-select-c1", "workload": "oltp_point_select", "threads": 1, "duration": 30, "warmup": 5},
            {"name": "read-only-c4", "workload": "oltp_read_only", "threads": 4, "duration": 45, "warmup": 5},
            {"name": "read-only-c16", "workload": "oltp_read_only", "threads": 16, "duration": 60, "warmup": 5},
            {"name": "write-only-c4", "workload": "oltp_write_only", "threads": 4, "duration": 45, "warmup": 5},
            {"name": "read-write-c1", "workload": "oltp_read_write", "threads": 1, "duration": 30, "warmup": 5},
            {"name": "read-write-c4", "workload": "oltp_read_write", "threads": 4, "duration": 45, "warmup": 5},
            {"name": "read-write-c16", "workload": "oltp_read_write", "threads": 16, "duration": 60, "warmup": 5},
        ],
    },
}
